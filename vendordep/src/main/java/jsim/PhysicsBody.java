 ...
 * <p>Usage example:
 * <pre>{@code
 * // PhysicsBody is created by PhysicsWorld, not directly:
 * PhysicsBody body = world.createBody(50.0); // 50 kg robot
 * body.setGravityEnabled(false);
 * body.setCollisionBox(0.7, 0.7, 0.35);      // width, depth, height in meters
 * body.setPosition(new Pose3d(3.0, 2.0, 0.175, new Rotation3d()));
 * body.setLinearVelocity(1.0, 0.0, 0.0);    // 1 m/s forward
 *
 * // Read back position after step:
 * Pose3d pos = body.position();
 * LinearVelocity3d vel = body.linearVelocity();
 * }</pre>
 */
