 ...
 * @brief 3D shot-solution generator using lookup-table interpolation and
...
class ShotCalculator3D {
 public:
  ...
  struct ShotParameters {
    /// @brief True when a valid solution was found within the configured
    /// distance range.
    bool is_valid{false};
  ...
  void setLookupTable(std::vector<TablePoint> points) {
    ...
    for (const auto& point : points) {
      if (!std::isfinite(point.distance_m) || !std::isfinite(point.hood_pitch_rad) || !std::isfinite(point.muzzle_speed_mps) ||
    ...
    std::sort(table_.begin(), table_.end(), [](const TablePoint& a, const TablePoint& b) { return a.distance_m < b.distance_m; });
  }
  ...
  ShotParameters calculateShot(const Vector3& flywheel_origin_m, const Vector3& robot_velocity_mps, const Vector3& target_position_m, double now_s) {
    ...
    if (table_.empty() || !finiteVector(flywheel_origin_m) || !finiteVector(robot_velocity_mps) || !finiteVector(target_position_m) ||
        !std::isfinite(now_s)) {
      return result;
    }
    ...
    const Vector3 delayed_origin = flywheel_origin_m + robot_velocity_mps * config_.phase_delay_s;
    ...
    if (valid) {
      saveRecentShotSample(now_s, lookahead_origin, target_position_m, distance_clamped, hood_pitch_rad, flywheel_speed_mps);
    }
 ...
 private:
  ...
  static bool finiteVector(const Vector3& v) { return std::isfinite(v.x) && std::isfinite(v.y) && std::isfinite(v.z); }
  ...
  static Config sanitizeConfig(const Config& config) {
    ...
    sanitized.recent_pose_band_m = std::max(sanitized.valid_distance_epsilon, sanitizeNonNegative(sanitized.recent_pose_band_m, 0.75));
    sanitized.recent_target_band_m = std::max(sanitized.valid_distance_epsilon, sanitizeNonNegative(sanitized.recent_target_band_m, 0.75));
    ...
    sanitized.ballistic_pitch_gain_radpm =
        std::clamp(std::isfinite(sanitized.ballistic_pitch_gain_radpm) ? sanitized.ballistic_pitch_gain_radpm : 0.035, 0.0, 0.4);
    sanitized.ballistic_speed_gain_mpspm =
        std::clamp(std::isfinite(sanitized.ballistic_speed_gain_mpspm) ? sanitized.ballistic_speed_gain_mpspm : 0.75, 0.0, 10.0);
    ...
    sanitized.projectile_drag_coefficient = std::clamp(sanitizeNonNegative(sanitized.projectile_drag_coefficient, 0.47), 0.0, 5.0);
  ...
  double clampedDistance(double distance_m) const { return std::clamp(distance_m, config_.min_distance_m, config_.max_distance_m); }
  ...
  Vector3 ballisticAcceleration(const Vector3& velocity_mps) const {
    ...
    const Vector3 drag_force_n =
        Vector3::dragForce(velocity_mps, config_.projectile_drag_coefficient, config_.projectile_reference_area_m2, config_.air_density_kgpm3) *
        config_.drag_scale;
    const Vector3 magnus_force_n = Vector3::magnusForce(velocity_mps, config_.assumed_spin_radps, config_.magnus_coefficient) * config_.magnus_scale;
    const Vector3 accel_mps2 = gravity_mps2 + (drag_force_n + magnus_force_n) * (1.0 / std::max(1e-9, config_.projectile_mass_kg));
    return finiteVector(accel_mps2) ? accel_mps2 : gravity_mps2;
  }

  BallisticSample simulateAtRange(const Vector3& origin_m, double yaw_rad, double pitch_rad, double speed_mps, double target_planar_range_m) const {
    BallisticSample sample{};
    sample.position_m = origin_m;

    const double cos_pitch = std::cos(pitch_rad);
    Vector3 velocity_mps(speed_mps * cos_pitch * std::cos(yaw_rad), speed_mps * cos_pitch * std::sin(yaw_rad), speed_mps * std::sin(pitch_rad));
    ...
    while (sample.time_s < config_.ballistic_max_time_s) {
      ...
      if (sample.travelled_range_m >= target_planar_range_m) {
        const double denom = std::max(config_.valid_distance_epsilon, sample.travelled_range_m - previous_range_m);
  ...
  void refineBallisticShot(const Vector3& origin_m, const Vector3& target_position_m, double yaw_rad, double& hood_pitch_rad,
    ...
    for (int i = 0; i < config_.ballistic_refinement_iterations; ++i) {
      const auto sample = simulateAtRange(origin_m, yaw_rad, hood_pitch_rad, flywheel_speed_mps, target_planar_range_m);
      ...
      flywheel_speed_mps += std::clamp(vertical_error_m * config_.ballistic_speed_gain_mpspm * 0.5, -0.6, 0.6);
  ...
  void trimRecentShotHistory(double now_s) {
    while (!recent_shot_samples_.empty() && now_s - recent_shot_samples_.front().timestamp_s > config_.recent_shot_history_window_s) {
      recent_shot_samples_.pop_front();
    }
  }

  const RecentShotSample* findRecentShotSample(const Vector3& robot_translation_m, const Vector3& target_position_m) const {
    const double pose_band = std::max(config_.valid_distance_epsilon, config_.recent_pose_band_m);
  ...
  void saveRecentShotSample(double now_s, const Vector3& robot_translation_m, const Vector3& target_position_m, double distance_m,
                            double hood_pitch_rad, double flywheel_speed_mps) {
    if (now_s - last_recent_sample_s_ < config_.recent_shot_sample_period_s) {
      return;
    }

    recent_shot_samples_.push_back({now_s, robot_translation_m, target_position_m, distance_m, hood_pitch_rad, flywheel_speed_mps});
