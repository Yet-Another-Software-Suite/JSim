...
struct alignas(16) Vector3 {
  ...
  Vector3 clamp(const Vector3& min, const Vector3& max) const noexcept {
    return Vector3(std::max(min.x, std::min(x, max.x)), std::max(min.y, std::min(y, max.y)), std::max(min.z, std::min(z, max.z)));
  }
  /**
   ...
   */
  static Vector3 lerp(const Vector3& a, const Vector3& b, double t) noexcept { return a * (1.0 - t) + b * t; }
  ...
  Vector3 cross(const Vector3& o) const noexcept { return {y * o.z - z * o.y, z * o.x - x * o.z, x * o.y - y * o.x}; }
   ...
   */
  Vector3 reflect(const Vector3& n) const noexcept { return *this - n * (2.0 * dot(n)); }
   ...
   */
  static Vector3 magnusForce(const Vector3& velocity, const Vector3& omega, double k = 1e-4) noexcept { return omega.cross(velocity) * k; }
  ...
  struct DragVector {
    ...
    bool isZero(double eps = 1e-12) const noexcept { return std::abs(x) < eps && std::abs(y) < eps && std::abs(z) < eps; }
  };
   ...
   */
  static DragForceDetails dragForceDetailed(const Vector3& v, double Cd, double A, double rho = 1.225,
    ...
    if (!std::isfinite(v.x) || !std::isfinite(v.y) || !std::isfinite(v.z) || !std::isfinite(Cd) || !std::isfinite(A) || !std::isfinite(rho) ||
    ...
    details.force = {details.direction.x * (-force_magnitude_n), details.direction.y * (-force_magnitude_n),
   ...
   */
  static Vector3 dynamicGravity(const Vector3& velocity, const Vector3& spin, double g = 9.81, double magnusCoeff = 1e-4,
