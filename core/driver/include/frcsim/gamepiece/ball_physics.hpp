...
/**
 * @file ball_physics.hpp
 * @brief 3D rigid-body ball simulator with drag, Magnus lift, and ground
 * contact.
 */
...
/** @defgroup gamepieces Gamepiece Physics */
...
class BallPhysicsSim3D {
 public:
   ...
   *
   * @note All values use SI units. Modified values take effect on the next
   * step() call.
   */
  struct Config {
    /// @brief Constant gravity vector applied each integration substep (m/s^2).
    Vector3 gravity_mps2{0.0, 0.0, -9.81};
    /// @brief Multiplier applied to gravity_mps2 after sanitization (clamped to
    /// [0, 5]).
    double effective_gravity_scale{1.0};
    /// @brief Air density used by drag force computation (kg/m^3).
    double air_density_kgpm3{1.225};
    /// @brief Additional scale factor applied to drag force (clamped to [0,
    /// 10]).
    double drag_scale{1.0};
    /// @brief Coefficient used by Vector3::magnusForce.
    double magnus_coefficient{1e-4};
    /// @brief Additional scale factor applied to Magnus force (clamped to [0,
    /// 10]).
    double magnus_scale{1.0};
    /// @brief World Z height for the flat ground plane (meters).
    double ground_height_m{0.0};
    /// @brief Exponential-style planar speed decay rate while on ground (1/s).
    double rolling_friction_per_s{1.2};
    /// @brief Minimum downward impact speed that triggers a bounce (m/s).
    double min_bounce_speed_mps{0.1};
    /// @brief Maximum internal integration substep size (seconds, clamped to
    /// [1e-4, 0.05]).
   ...
   *
   * @note All values are sanitized on construction; non-positive mass/radius
   *       values are replaced with safe defaults.
   */
  struct BallProperties {
    /// @brief Ball mass in kilograms.
    double mass_kg{0.27};
    /// @brief Ball radius in meters.
    double radius_m{0.12};
    /// @brief Dimensionless drag coefficient (Cd).
    double drag_coefficient{0.47};
    /// @brief Reference frontal area used for drag in square meters.
    double reference_area_m2{0.045};
    /// @brief Coefficient of restitution for ground impacts in [0, 1].
    double restitution{0.45};

    /// @brief Foam compression stiffness in N/m (for high-density foam
    /// gamepieces).
  ...
  explicit BallPhysicsSim3D(const Config& config) : config_(sanitizeConfig(config)) {}
 ...
 private:
  ...
  static BallProperties sanitizeBallProperties(const BallProperties& props) {
    ...
    sanitized.reference_area_m2 = sanitizeNonNegative(sanitized.reference_area_m2, std::acos(-1.0) * sanitized.radius_m * sanitized.radius_m);
  ...
  Vector3 computeAcceleration(const Vector3& velocity_mps) const {
    ...
    const Vector3 drag_force_n =
        Vector3::dragForce(velocity_mps, ball_properties_.drag_coefficient, ball_properties_.reference_area_m2, config_.air_density_kgpm3) *
        config_.drag_scale;
    const Vector3 magnus_force_n = Vector3::magnusForce(velocity_mps, state_.spin_radps, config_.magnus_coefficient) * config_.magnus_scale;
    const Vector3 accel_mps2 = gravity_mps2 + (drag_force_n + magnus_force_n) * (1.0 / std::max(1e-9, ball_properties_.mass_kg));
