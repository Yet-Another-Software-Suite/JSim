 ...
 * @brief PhysicsWorld step loop, collision resolution, and object factory
 * methods.
 *
 * The step() method is the heart of the simulation: it applies aerodynamic
 * forces, integrates bodies, solves assembly joint constraints, advances
 * gamepiece physics, and finally resolves all pairwise collisions (gamepiece-
 * body and gamepiece-gamepiece). Order matters — gamepiece physics runs after
 * body integration so carried pieces follow their carrier correctly.
 */
...
BallPhysicsSim3D& PhysicsWorld::createBall(const Gamepiece::Config& config, const Gamepiece::Properties& properties) {
...
Gamepiece& PhysicsWorld::createGamepiece(const Gamepiece::Config& config, const Gamepiece::Properties& properties) {
...
void PhysicsWorld::setMaterialInteraction(const MaterialInteraction& interaction) {
  // NOTE: IDs are canonicalised (min,max) so that (A,B) and (B,A) resolve to
  // the same entry; this makes lookup symmetric without a second hash bucket.
  const std::int32_t a = std::min(interaction.material_a_id, interaction.material_b_id);
  const std::int32_t b = std::max(interaction.material_a_id, interaction.material_b_id);
  for (auto& existing : material_interactions_) {
    const std::int32_t ea = std::min(existing.material_a_id, existing.material_b_id);
    const std::int32_t eb = std::max(existing.material_a_id, existing.material_b_id);
    if (ea == a && eb == b) {
...
void PhysicsWorld::clearMaterialInteractions() { material_interactions_.clear(); }

bool PhysicsWorld::shouldInteract(std::uint32_t layer_a, std::uint32_t mask_a, std::uint32_t layer_b, std::uint32_t mask_b) const {
  return ((layer_a & mask_b) != 0u) && ((layer_b & mask_a) != 0u);
}

const PhysicsWorld::MaterialInteraction* PhysicsWorld::findMaterialInteraction(std::int32_t material_a_id, std::int32_t material_b_id) const {
  ...
  for (const auto& interaction : material_interactions_) {
    ...
    const std::int32_t ia = std::min(interaction.material_a_id, interaction.material_b_id);
    const std::int32_t ib = std::max(interaction.material_a_id, interaction.material_b_id);
    if (ia == a && ib == b) {
      return &interaction;
    }
  }
...
void PhysicsWorld::step() {
  ...
  // Returns a sphere-equivalent collision radius for a body based on its
  // aerodynamic geometry. Used for broadphase contact testing against
  // boundaries and gamepieces; a single radius is cheaper than shape-specific
  // GJK and sufficient for FRC game accuracy requirements.
  auto body_collision_radius_m = [&](const RigidBody& body) {
    ...
    const double fallback_area_m2 = std::max(0.0, config_.default_drag_reference_area_m2);
  ...
  auto resolve_boundary_contact = [&](RigidBody& body) {
    ...
    const double body_restitution = body_material ? body_material->coefficient_of_restitution : 0.4;
    const double body_mu_kinetic = body_material ? body_material->coefficient_of_friction_kinetic : 0.6;
    ...
    for (const auto& boundary : boundaries_) {
      ...
      if (!shouldInteract(body.collisionLayer(), body.collisionMask(), boundary.collision_layer_bits, boundary.collision_mask_bits)) {
        continue;
      }
      ...
      if (boundary.type == BoundaryType::kPlane || boundary.type == BoundaryType::kWall) {
        ...
        const bool inside_xy = std::abs(rel_local.x) <= std::max(0.0, half.x) && std::abs(rel_local.y) <= std::max(0.0, half.y);
      ...
      double restitution = std::clamp(0.5 * (boundary.restitution + body_restitution), 0.0, 1.0);
      double friction = std::max(0.0, 0.5 * (boundary.friction_coefficient + body_mu_kinetic));

      // NOTE: Per-material-pair overrides win unconditionally; if a pair
      // entry exists it replaces the averaged body+boundary values entirely
      // rather than blending again, so the table provides exact control.
      if (const auto* pair = findMaterialInteraction(body.materialId(), boundary.material_id)) {
      ...
      if (vt_mag > 1e-9) {
        const double friction_impulse_ratio = std::clamp(friction * (1.0 + restitution) * std::abs(vn) / vt_mag, 0.0, 1.0);
  ...
  auto step_body = [&](RigidBody& body) {
    if (config_.enable_aerodynamics && !body.flags().is_kinematic) {
      DragModel drag_model(config_.default_drag_coefficient, config_.default_drag_reference_area_m2, config_.air_density_kgpm3,
                           config_.linear_drag_coefficient_n_per_mps);
      body.applyForce(drag_model.computeForce(body));

      const Vector3 magnus = Vector3::magnusForce(body.linearVelocity(), body.angularVelocity(), config_.magnus_coefficient);
    ...
    body.integrate(dt_s, config_.integration_method, config_.gravity_mps2, config_.linear_damping_per_s, config_.angular_damping_per_s);
  ...
  // Resolve gamepiece <-> rigid-body collisions after the gamepiece has
  // advanced its own physics. NOTE: Collisions are resolved after
  // gamepiece.step() so that a carried gamepiece has already been moved to its
  // carrier's position before the collision check runs — preventing spurious
  // contacts with the carrying body.
  for (auto& gamepiece : gamepieces_) {
  ...
  // Resolve gamepiece <-> gamepiece collisions (pairwise) so gamepieces
  // interact with each other. We only consider airborne gamepieces here to
  // match rigid-body collision handling. NOTE: O(n^2) pairwise loop is
  // acceptable because FRC matches typically have fewer than ~20 gamepieces on
  ...
  for (std::size_t i = 0; i < gamepieces_.size(); ++i) {
    for (std::size_t j = i + 1; j < gamepieces_.size(); ++j) {
      ...
      if (a.getGamepieceState() != Gamepiece::State::kAirborne || b.getGamepieceState() != Gamepiece::State::kAirborne) {
        continue;
      }
