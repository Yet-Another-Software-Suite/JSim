...
from .config import DEFAULT_MATERIAL, ExportFormat
from .materials import MaterialSystem
from .mechanisms import GroupedMechanism
...
class CADExporter:
    ...
    def export_to_json(
        ...
        try:
            export_data = {"version": "1.0", "format": "jsim_cad", "mechanisms": []}
    ...
    def _mechanism_to_json(
        self, mechanism: GroupedMechanism, include_geometry: bool
    ) -> Dict[str, Any]:
    ...
    def export_to_java_codegen(
        ...
        try:
            ...
            for mechanism in mechanisms:
                mech_class = self._generate_java_mechanism_class(
                    mechanism, package_name
                )
                class_file = (
                    output_dir / f"Generated{mechanism.name.replace('-', '_')}.java"
                )
    ...
    def _generate_java_world_setup(
        self, mechanisms: List[GroupedMechanism], package_name: str
    ) -> str:
...
"""
    ...
    def _generate_java_mechanism_class(
        self, mechanism: GroupedMechanism, package_name: str
    ) -> str:
...
"""
    ...
    def export_summary_report(
        self, mechanisms: List[GroupedMechanism], output_path: str
    ) -> bool:
        ...
        try:
            ...
            total_mass = sum(sum(c.mass for c in m.components) for m in mechanisms)
    ...
    def export(
        ...
        **kwargs,
        ...
        if format == ExportFormat.JSON:
            ...
            return self.exporter.export_to_java_codegen(
                mechanisms, output_path, **kwargs
            )
        else:
