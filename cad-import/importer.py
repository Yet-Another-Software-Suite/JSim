...
from .mechanisms import GroupedMechanism, MechanismExtractor
...
class OnShapeCADImporter:
    ...
    def import_gltf(self, gltf_path: str) -> bool:
        ...
        try:
            ...
            if gltf_path.suffix.lower() == ".gltf":
                ...
                logger.info(
                    "Binary glTF (.glb) import requires binary parsing - converting to JSON model"
                )
    ...
    def _process_gltf_data(self, gltf_data: Dict[str, Any]):
        ...
        if components:
            self.mechanism_extractor.extract_mechanism_from_group(
                group_name="imported_assembly", components=components, joints=[]
            )
    ...
    def _process_glb_file(self, glb_path: Path):
        ...
        """
        logger.warning(
            "GLB binary parsing not yet implemented - assuming single assembly"
        )
    ...
    def get_mechanisms_to_export(self) -> List[GroupedMechanism]:
        ...
        """
        mechanisms = self.mechanism_extractor.get_mechanisms_to_simulate(
            self.accuracy_level
        )
        logger.info(
            f"Filtered to {len(mechanisms)} mechanisms for {self.accuracy_level.value} accuracy"
        )
        return mechanisms
...
class FieldCADImporter(OnShapeCADImporter):
    ...
    def __init__(self, field_year: Optional[int] = None):
        ...
        )  # Field elements need high accuracy
