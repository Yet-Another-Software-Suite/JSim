...
class MaterialSystem:
    ...
    def get_material_properties(
        self, material_name: Optional[str]
        ...
        logger.debug(
            f"Material '{material_name}': ρ={density} kg/m³, μ={friction}, e={restitution}"
        )
    ...
    def is_realistic_mass(
        self, mass: float, expected_range: Tuple[float, float] = None
    ) -> bool:
        ...
        if mass < min_mass or mass > max_mass:
            logger.warning(
                f"Unrealistic mass detected: {mass} kg (expected {min_mass}-{max_mass} kg)"
            )
            return False
        return True

    def add_custom_material(
        self, name: str, density: float, friction: float, restitution: float
    ):
        ...
        logger.info(
            f"Added custom material: {name} (ρ={density}, μ={friction}, e={restitution})"
        )
