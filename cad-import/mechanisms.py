...
class GroupedMechanism:
    ...
    def validate(self) -> tuple[bool, List[str]]:
        ...
        if self.root_component not in component_names:
            issues.append(
                f"Root component '{self.root_component}' not in components list"
            )
        ...
        for joint in self.joints:
            if joint.parent_component not in component_names:
                issues.append(
                    f"Joint '{joint.name}' references unknown parent '{joint.parent_component}'"
                )
            if joint.child_component not in component_names:
                issues.append(
                    f"Joint '{joint.name}' references unknown child '{joint.child_component}'"
                )
        ...
        if zero_mass:
            issues.append(
                f"Zero mass components: {zero_mass} (will be treated as fixed)"
            )
...
class MechanismExtractor:
    ...
    def extract_mechanism_from_group(
        ...
        if is_valid:
            ...
            logger.info(
                f"Extracted mechanism: {group_name} ({mech_type.value}, {len(extracted_components)} components)"
            )

        return mechanism if is_valid else None

    def _infer_mechanism_type(
        self, components: List[Component], joints: List[Joint]
    ) -> MechanismType:
    ...
    def get_mechanisms_to_simulate(self, accuracy_level) -> List[GroupedMechanism]:
        ...
        for mechanism in self.mechanisms:
            ...
            has_meaningful_mass = any(
                c.mass >= config.min_mass_threshold for c in mechanism.components
            )
